...
class FairRankingTask(object):
    ...
    def __init__(self, sequence_file, groundtruth_file, group_annotations_file):
        ...
        self.target_ranking = self.target_ranking()
        self.all_groups = set([g for groups in self.document_to_groups.values() 
            for g in groups])


    def target_ranking(self):
        target_ranking = []
        for seq_id in self.sequence:
            q_list = self.sequence[seq_id]
            for q_set in q_list:
                target_result = {}
                qid = q_set[1]
                target_result['q_num'] = str(seq_id) + '.' + str(q_set[0])
                target_result['qid'] = qid
                docDict = self.groundtruth[qid]
                rel_1 = []
                rel_0 = []
                for doc_id in docDict:
                    if docDict[doc_id] == 1:
                        rel_1.append(doc_id)
                    else:
                        rel_0.append(doc_id)
                random.shuffle(rel_1)
                random.shuffle(rel_0)
                rel_1.extend(rel_0)
                target_result['ranking'] = rel_1
                target_ranking.append(target_result)

        target_data = defaultdict(dict)
        for ranking_data in target_ranking:
            seq_id, q_num = ranking_data['q_num'].split('.')
            target_data[int(seq_id)][int(q_num)] = ranking_data['ranking']
        return target_data
    ...
    def groups_exposure(self, seq_id, submission, gamma):
        ...
        for q_num, q_id in self.sequence[seq_id]:
            ...
            for i, doc_id in enumerate(submission.rankings[seq_id][q_num]):
                ...
                for g in self.document_to_groups[doc_id]:
                    # print(self.document_to_groups[doc_id])
        ...
        return dict([(g, exp/150) for g, exp in g_exps.items()])


    def target_groups_exposure(self, seq_id, gamma):

        g_exps = dict([(g, 0.) for g in self.all_groups])

        for q_num, q_id in self.sequence[seq_id]:
            stopped_until_now = 1.
            for i, doc_id in enumerate(self.target_ranking[seq_id][q_num]):
                if doc_id not in self.document_to_groups:
                    continue
                for g in self.document_to_groups[doc_id]:
                    g_exps[g] += (gamma ** i) * stopped_until_now * \
...
def l2_loss(seq_id, task, submission, gamma):
    ...
    target_groups_exposure = task.target_groups_exposure(seq_id, gamma)
    return math.sqrt(sum([(groups_exposure[g] - target_groups_exposure[g])**2
        for g in task.all_groups]))
...
if __name__ == '__main__':
    ...
    run_files_prefix = 'fairRunsNew/'
    run_files = [
        'dispImBN_r0_g100_c0.jsonl',
        'dispImKL_r0_g100_c0.jsonl',
        'dispImKLZ_r0_g100_c0.jsonl',
        'bm25.jsonl',
        'dispImBN_r0_g0_c100.jsonl',
        'dispImKL_r0_g0_c100.jsonl',
        'dispImKLZ_r0_g0_c100.jsonl',
        #'statPar_r0_g0_c100.jsonl',
        #'statPar_r0_g100_c0.jsonl',
        'random.jsonl',
    ]
    ...
    for run in run_files:
        with open('eval_results_new/%s/%s' % (args.group_definition, run), 'w') as f_out:
    ...
    # fig = plt.figure()
    plt.savefig('plots/%s/performance-all-new.pdf' % args.group_definition)
    # plt.show()
